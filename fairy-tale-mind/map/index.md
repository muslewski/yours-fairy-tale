...
| [[payload-backend]] | active | ⚠ stale | Payload v3 backend — the in-app CMS, /admin panel, REST/GraphQL API, the admins native-auth collection, the Waitlist collection, Vercel Blob media storage (pass-through), and production boot (env validation + migrate-on-deploy). |
