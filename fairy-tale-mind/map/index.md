...
<!-- mind:counts zones=17 stale=7 debt=11 -->
# Map Index
...
| [[studio]] | active | ⚠ stale | The staff order panel at /studio — dashboard (revenue totals, needs-attention queue), order list, and a per-order workstation (status workflow with guardrails, delivery promise, browser-to-Blob video uploads), gated by the Payload admins login. Same brand chrome as the customer area; Payload /admin remains the fallback tool. |
